...
def get_api_result(url, data):
    ...
    try:
        ...
        if res.status_code == 200:
            ...
            # cookies = dict(cookies, **res.cookies.get_dict())
            # print("COOKIES:" + cookies.__str__())
        ...
        print('\033[31m requests Fail.\033[0m')
        # traceback.print_exc()


def buy(offer):
    print('Buy ' + offer['items'][0]['_tpl'] + ' ( ' + all_item_list[offer['items'][0]['_tpl']]['_props'][
    ...
    if spent_time < start_from:
        ...
        # print('Wait ' + str(to_wait / 100))
        time.sleep(to_wait / 100)
    ...
    if result_data['err'] == 1505:
        print("\033[31m ... OUT OF SPACE\033[0m")
        exit()

    if result_data['err'] == 0:
        # offer was sold out
        if len(result_data['data']['badRequest']) > 0:
            print('\033[31m ... SOLD OUT\033[0m')
            todo = {}
        # added new item to inventory
        elif len(result_data['data']['items'].keys()) > 0:
            # print('GOT IT !')
            print('\033[32m         + ' + format(int(offer['itemsCost'] * 0.75) - offer['summaryCost'],
                                                 ',') + '\033[0m')
        ...
        print('\033[31m ... ErrorCode: ' + str(result_data['err']) + '\033[0m')
...
def update_profile():
    ...
    if Balance - oldBalance > 0:
        print('\033[32m[' + timeStr + '] Balance: ' + format(Balance, ',') + '\033[0m')
    else:
        print('\033[31m[' + timeStr + '] Balance: ' + format(Balance, ',') + '\033[0m')
...
if __name__ == '__main__':
    ...
    # out put
    # f = open("item_list.txt", 'w', encoding='UTF-8')
    # for key in all_item_list:
    #     if 'CreditsPrice' in all_item_list[key]['_props']:
    #         if all_item_list[key]['_props']['CreditsPrice'] / (all_item_list[key]['_props']['Width'] * all_item_list[key]['_props']['Height']) / 100 * 75 >= 25000:
    #             f.write("{\"%s\",\"%s\"},\n" % (key, all_item_list[key]['_props']['ShortName']))
    # f.write("%s\n%s\n" % (key, all_item_list[key]['_props']['ShortName']))
    ...
    if NPC['working'] is not True:
        print('Therapist off duty')
        exit()

    while 1:
        print('\033[34mPHPSESSID: ' + cookies['PHPSESSID'] + '\033[0m')
        # WishList = PMC['WishList']:
        print('WishList Len: ' + str(len(WishList)))
